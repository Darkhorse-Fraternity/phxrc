...
function navigationState(state: Object = initialNavLoginState, action) {
    switch (action.type) {
        case NAV_PUSH: {
            ...
            const direction = action.state.direction?action.state.direction:'horizontal'
            if (state.routes[state.index].key === (action.state && action.state.key)) return state
            return {
                ...NavigationStateUtils.push(state, action.state),
                direction,
            }
        }
        ...
        case NAV_POP:
            ...
            return {
                ...
                routes: popRoutes,
                // direction:action.state.direction||'horizontal',
            }
